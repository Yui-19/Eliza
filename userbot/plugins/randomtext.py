...
async def _(event):  # sourcery no-metrics
    ...
    if x == 5:
        await event.edit(
            '`"Think of purchases as a percentage of your budget or account balance rather than their actual cost"`'
        ) 
    if x == 6:
        await event.edit(
    ...
    if x == 10:
        await event.edit(
            '`"Never leave water bottles , reading glasses , or anything else that can focus light in a spot that could get direct sunlight ! A significant number of house or vehicle fires happen every year because of this"`'
        )
    if x == 11:
        await event.edit(
    ...
    if x == 72:
        await event.edit(
            '`"Whenever your smoke alarm goes off , give your dog a treat 1 They\'ll associate the alarm with the treat ; so when the alarm goes off for real , your dog will come right to you"`'
        )
    if x == 73:
        await event.edit(
...
async def _(event):  # sourcery no-metrics
    ...
    if x == 6:
        await event.edit(
            '`"Your age in years is how many times you have circled the sun , but your age in months is how many times the Moon has circled you"`'
        )
    if x == 7:
        await event.edit(
            '`"Biting your tongue while eating is a perfect example of how you can still screw up , even with decades of experience"`'
    ...
    if x == 100:
        await event.edit(
            "`Do you know , When you see a bearded goat , that means you just saw a *smarter goat* than you...`"
        )
    if x == 101:
        await event.edit(
    ...
    if x == 103:
        await event.edit(
            "`Do you know , there's a huge difference between *cartoon and anime*...`"
        )
    if x == 104:
