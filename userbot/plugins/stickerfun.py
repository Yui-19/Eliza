    ...
    if not text:
        if event.is_reply:
            ...
            return await edit_delete(
                event, "What is honk supposed to say ? Give some text"
            )
    text = deEmojify(text)
    await event.delete()
...
async def twt(event):
    ...
    if not text:
        if event.is_reply:
            ...
            return await edit_delete(
                event, "What am I supposed to tweet ? Give some text"
            )
    text = deEmojify(text)
    await event.delete()
    await hide_inlinebot(event.client, bot_name, text, event.chat_id, reply_to_id)
...
async def quby(event):
    ...
    if not text:
        return await edit_delete(
            event, "What is quby supposed to say ? Give some text"
        )
...
async def knife(event):
    ...
    if not text:
        return await edit_delete(
            event, "What is knife supposed to say ? Give some text"
        )
    await edit_delete(event, "`Wait , processing...`")
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
...
async def doge(event):
    ...
    if not text:
        return await edit_delete(
            event, "What is doge supposed to say ? Give some text"
        )
    await edit_delete(event, "`Wait , processing...`")
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
