...
async def tmuter(event):  # sourcery no-metrics
    ...
    try:
        ...
        return await catevent.edit("`Oh my mute logic broke !`")
...
async def tban(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if not reason:
        return await catevent.edit("You haven't mentioned time\n\nCheck `.help tban`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    cattime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ...
    try:
        ...
        return await catevent.edit(
            "`I dont have message nuking rights\n\nBut still he was banned`"
        )
