...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    if output is None:
        return await edit_delete(catevent, "Unable to make spin gif")
...
async def _(event):
    ...
    if not reply:
        return await edit_delete(
            event, "Reply to any sticker or media to convert it to image"
        )
...
async def _(event):
    ...
    if not reply:
        return await edit_delete(
            event, "Reply to any image or media to convert it to sticker"
        )
    output = await _cattools.media_to_pic(event, reply)
    if output[1] is None:
        return await edit_delete(
...
@catub.cat_cmd(
    ...
    info={
        ...
        "description": "By default will use -i as flag",
        "flags": {
            "-r": "Right rotate gif",
            "-l": "Left rotate gif",
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    if not reply or not mediatype or mediatype not in ["Photo", "Sticker"]:
        return await edit_delete(event, "Reply to photo or sticker to make it gif")
    ...
    catevent = await edit_or_reply(event, "Making gif from the relied media...")
    ...
    if output is None:
        await edit_delete(
            catevent, "There was some error in the media I can't format it to gif"
        )
...
async def _(event):
    ...
    if mediatype and mediatype != "video":
        return await edit_delete(event, "Reply to video to convert it to gif")
    args = event.pattern_match.group(1)
    if not args:
        args = 2.0
    else:
        try:
            ...
            args = 2.0
    catevent = await edit_or_reply(event, "Converting into gif..")
    ...
    if result is None:
        return await edit_delete(event, "I couldn't convert it to gif")
