...
if __ is not None:
    if "uc?id=" in G_DRIVE_FOLDER_ID:
        LOGS.info("G_DRIVE_FOLDER_ID is not a valid folder url...")
...
async def download(event, gdrive, service, uri=None):  # sourcery no-metrics
    ...
    if uri:
        ...
        try:
            ...
            """ Assuming newest files are the cancelled one """
            newest = max(names, key=os.path.getctime)
            os.remove(newest)
            reply += (
                "FILE - CANCELLED\n\n"
                "Status : `Okay - received signal cancelled`"
            )
            return reply
        else:
        ...
        if os.path.isfile(required_file_name):
            ...
            try:
                ...
                reply += (
                    ...
                    "Status : `Okay - successfully uploaded`\n\n"
                )
                return reply
...
async def gdrive_download(
    ...
    await gdrive.edit(
        ...
        f"Path   : `{file_path}`\n"
        "Status : Okay - successfully downloaded"
    )
...
async def upload(gdrive, service, file_path, file_name, mimeType, dir_id=None):
    ...
    body = {
        "name": file_name,
        "description": "Uploaded from telegram using cat userbot",
        "mimeType": mimeType,
    ...
    while not complete:
        ...
        try:
            ...
            if " not found " in str(e) or "'file'" in str(e):
                ...
                await event.edit(
                    "Download auto canceled :\n`{}`\nYour torrent or link is dead".format(
                        file.name
                    )
                )
...
@catub.cat_cmd(
    ...
    info={
        ...
        "description": "Generate token to enable all command google drive service\n\nThis only need to run once in life time",
        "usage": "{tr}gauth",
    },
)
async def generate_credentials(gdrive):
    ...
    if helper.get_credentials(str(hmm)) is not None:
        await edit_or_reply(gdrive, "`You have already authorized token...`")
...
@catub.cat_cmd(
    ...
    info={
        ...
        "description": "Reset your token if something bad happened or change drive account",
        "usage": "{tr}greset",
    },
)
...
@catub.cat_cmd(
    ...
    info={
        ...
        "flags": {
            ...
            "p": "Use flag `-p parents-folder_id` for files or folder in given folder in gdrive",
        },
...
@catub.cat_cmd(
    ...
    info={
        ...
        "description": "To create or delete or check folders or files in gdrive",
        "options": {
            "mkdir": "Create gdrive folder",
            "info": "To get file or folder details of our gdrive",
            "rm": "Delete files or folders in gdrive ! Can't be undone , this method skipping file trash , so be caution..",
        },
        "usage": [
...
async def google_drive_managers(gdrive):  # sourcery no-metrics
    ...
    for name_or_id in f_name:
        ...
        if exe == "mkdir":
            ...
            try:
                """Try if given value is a name not a folderid or fileid"""
                f = result.get("files", [])[0]
                f_id = f.get("id")
            except IndexError:
                """If failed assumming value is folderid or fileid"""
                f_id = name_or_id
                try:
                    f = await get_information(service, f_id)
                except Exception as e:
                    reply += f"[FILE OR FOLDER - ERROR]\n\nStatus : `Bad`\nReason : `{e}`\n"
            ...
            try:
                ...
                try:
                    ...
                    continue
            """If exists parse file or folder information"""
...
@catub.cat_cmd(
    ...
    info={
        "header": "Upload files or folders to gdrive",
        "description": "Upload file from local or uri or url or drivelink into google drive"
...
async def google_drive(gdrive):  # sourcery no-metrics
    ...
    if not value and not gdrive.reply_to_msg_id:
        return await edit_or_reply(gdrive, "`What should I do idiot ?`")
    elif value and gdrive.reply_to_msg_id:
        await edit_or_reply(
            gdrive,
            "[UNKNOWN - ERROR]\n\n"
            "Status : `Failed`\n"
            "Reason : `Confused to upload file or the replied message or media`",
        )
        return None
    service = await create_app(gdrive)
    event = gdrive
    ...
    if os.path.isfile(value):
        ...
        if not uri and not event.reply_to_msg_id:
            await gdrive.edit(
                "[VALUE - ERROR]\n\n"
                "Status : `Bad`\n"
                "Reason : Given value is not url nor file or folder path "
                "If you think this is wrong , maybe you use .gd with multiple "
                "Value of files or folders , e.g `.gd <filename1> <filename2>` "
                "for upload from files or folders path this doesn't support it"
            )
            return False
...
async def set_upload_folder(gdrive):
    ...
    if G_DRIVE_FOLDER_ID is not None:
        ...
        await gdrive.edit(
            "[FOLDER - SET]\n\n" "Status : `Okay - using G_DRIVE_FOLDER_ID now`"
        )
        return None
    try:
        GDRIVE_.parent_id = ""
...
async def set_upload_folder(gdrive):
    ...
    try:
        ...
        if True in [c1 or c2]:
            ...
            await gdrive.edit(
                "[PARENT - FOLDER]\n\n" "Status : `Okay - successfully changed`"
            )
            return None
        await gdrive.edit(
