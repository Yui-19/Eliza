...
@catub.cat_cmd(
    ...
    info={
        ...
        "description": "Kang's the sticker or image or video or gif or webm file to the specified pack and uses the emoji ('s) you picked",
...
async def pussycat(args):
    ...
    if message and message.media:
        if "video/mp4" in message.media.document.mime_type:
            ...
            await edit_delete(args, "`Reply to video or gif...`")
            return
    else:
...
async def pic2packcmd(event):
    ...
    if not reply or not mediatype or mediatype not in ["Photo", "Sticker"]:
        return await edit_delete(event, "Reply to photo or sticker to make pack")
    ...
    if image[1] is None:
        return await edit_delete(
            image[0], "Unable to extract image from the replied message"
        )
    ...
    async with event.client.conversation(chat) as conv:
        ...
        for packname in ending.raw_text.split():
            ...
            if stick_pack_name.startswith("https://t.me/"):
                break
        await catevent.edit(
            f"Successfully created the pack for the replied media : [{args}]({stick_pack_name})"
        )
...
@catub.cat_cmd(
    ...
    info={
        ...
        "description": "Gets info about the sticker pack",
        "usage": "{tr}stkrinfo",
    },
)
