...
@catub.cat_cmd(
    ...
    info={
        "header": "To compress the files and folders",
...
async def zip_file(event):
    ...
    await mone.edit(
        f"Zipped the path `{input_str}` into `{filepath+'.zip'}` in {ms} seconds"
    )
...
@catub.cat_cmd(
    ...
    info={
        "header": "To compress the files and folders to tar file",
...
async def tar_file(event):
    ...
    ms = (end - start).seconds
    await mone.edit(
        f"Created a tar file for the given path {input_str} as `{destination}` in {ms} seconds"
    )
